...
export const TagsInput = React.forwardRef<HTMLDivElement, TagsInputProps>(
  ({ children, value, onValueChange, placeholder, maxItems, minItems, className, dir, ...props }, ref) => {
    ...
    const setInputValue = props.onInputChange ?? setInternalInputValue
    ...
    return (
      <TagInputContext.Provider
        ...
        <div
          ...
          dir={dir}
          className={cn(
            'flex items-center flex-wrap gap-1 py-1 px-2 rounded-md bg-background overflow-hidden ring-1 ring-muted',
            {
          ...
          <input
            ...
            onPointerDownCapture={() => {
              const val = inputValue.trim()
              if (val && val !== lastCommitted && !value.includes(val)) {
                lastCommitted = val
                onValueChange([...value, val])
                setInputValue('')
              }
            ...
            className={cn(
              'outline-0 border-none h-7 text-sm min-w-fit flex-1 bg-background-light focus-visible:outline-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-0 placeholder:text-muted-foreground px-1',
